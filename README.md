...
`./run` builds automatically on its first launch. A relative file path is resolved from the directory where you invoke `run`. Drop a file onto the window, choose **OPEN**, or paste a path or URL in the bottom field and press Enter or **PLAY**. Sources supported by your libmpv installation can be played.
...
| Volume and mute | Volume slider, mute button, or M |
| Fullscreen | F, **FULL**, or Escape to leave fullscreen |

Arrow keys retain their normal editing behavior while the source field has focus. The player reads the active Omarchy `colors.toml` palette and fontconfig monospace font while running, so changing the Omarchy theme or monospace font updates the interface.
